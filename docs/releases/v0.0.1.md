...
If you already have an AgentMux Hub URL, open the landing page, generate a join
signal, then run the generated Worker or Control command:
...
To self-host Hub with Docker:

```bash
docker run -d --name agentmux --restart unless-stopped -p 8081:8081 ghcr.io/kinboyw/agentmux:0.0.1
```

Or self-host Hub from the release binary:
